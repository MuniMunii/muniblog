      ...
      <AnimatePresence>
        {isMobile && isOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute size-full top-0 left-0 bg-black/80 flex justify-center items-center z-40"
          >
            <div className="pointer-events-auto flex flex-col item-center justify-center text-2xl gap-2">
              {navList.map((link) => {
                return (
                  <Link
                    className={`py-2 px-4 relative w-fit`}
                    key={link.href}
                    href={link.href}
